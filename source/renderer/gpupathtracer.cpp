...
namespace dx12_backend
{
	...
	static ID3D12Resource* create_resource_texture_2d(const wchar_t* name, DXGI_FORMAT format,
		u32 width, u32 height, u32 mip_count, D3D12_RESOURCE_STATES initial_state, D3D12_RESOURCE_FLAGS flags)
	{
		...
		resource_desc.Width = (u64)width;
		resource_desc.Height = height;
		resource_desc.DepthOrArraySize = 1;
		resource_desc.MipLevels = mip_count;
		resource_desc.SampleDesc.Count = 1;
		resource_desc.Flags = flags;

		ID3D12Resource* texture = nullptr;
		DX_CHECK_HR(g_dx12->d3d12_device->CreateCommittedResource(&heap_props, D3D12_HEAP_FLAG_NONE,
			&resource_desc, initial_state, nullptr, IID_PPV_ARGS(&texture)));
...
namespace gpupathtracer
{
	...
	void init(memory_arena_t* arena)
	{
		...
		inst->render_target = dx12_backend::create_resource_texture_2d(L"Final Color Texture",
			DXGI_FORMAT_R8G8B8A8_UNORM, g_renderer->render_width, g_renderer->render_height, 1,
		...
		dx12_backend::g_dx12->d3d12_device->CreateUnorderedAccessView(inst->render_target, nullptr, &uav_desc, inst->render_target_uav.cpu);
	}
	...
	void render()
	{
		...
		// we simply allocate from that scratch heap and make a UAV on the spot before clearing.
